    ...
    async updateTimeout() {
     let finalTimeout = 0
      switch(this.timeout) {
        case 'min':
          finalTimeout = 60
          this.timeout = { state: this.$t('config.autoLogout.time.min'), key: 'min' }
          break;
        case 'min5':
          finalTimeout = 300
          this.timeout = { state: this.$t('config.autoLogout.time.min5'), key: 'min5' }
          break;
        case 'hour':
          finalTimeout = 3600
          this.timeout = { state: this.$t('config.autoLogout.time.hour'), key: 'hour' }
          break;
        case 'hour6':
          finalTimeout = 21600
          this.timeout = { state: this.$t('config.autoLogout.time.hours6'), key: 'hour6' }
          break;
        case 'day':
          finalTimeout = 86400
          this.timeout = { state: this.$t('config.autoLogout.time.day'), key: 'day' }
          break; 
      }
