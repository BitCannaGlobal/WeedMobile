<template>
      ...
      <v-row>
        ...
        <v-col cols="6">
          <v-btn
            ...
            >{{ $t("accounts.importAccount") }}</v-btn
          >
        </v-col>
      </v-row>
    </v-card-text>
  ...
  <h4 v-if="allWallets.length > 0" class="ma-4">
    {{ $t("accounts.selectWallet") }}
  </h4>
  <v-card
    ...
    <v-card-actions>
      <v-col class="text-right">
        <v-btn
          color="#0FB786"
          variant="outlined"
          @click.stop="editNowModal(i)"
          >{{ $t("accounts.btnEditAccount") }}</v-btn
        >
        <v-btn color="red" variant="outlined" @click="openDeleteWallet()">{{
          $t("accounts.btnDeleteAccount")
        }}</v-btn>
      </v-col>
    </v-card-actions>
  </v-card>
    ...
    <v-card>
      ...
      <v-list lines="two" subheader>
        <v-list-item
          ...
          :subtitle="$t('accounts.mdlImportAccount.subtitle')"
        ></v-list-item>
      </v-list>
  ...
  >
    <v-card>
      ...
      <v-list lines="two" subheader>
        <v-list-item
          :title="$t('addressBook.info')"
          :subtitle="$t('accounts.viewMnemonic.subtitle')"
        >
        </v-list-item>
      </v-list>
  ...
  <v-bottom-sheet v-model="deleteWallet" inset>
    <v-card class="text-center bitcannaFont">
      <v-card-text>
        <v-btn variant="text" @click="deleteWallet = !deleteWallet">
  ...
  <div class="text-center">
    <v-bottom-sheet v-model="editNow" inset>
      <v-card class="text-center bitcannaFont">
        <v-card-text>
          <v-btn variant="text" @click="editNow = !editNow">
...
export default {
  ...
  data() {
    return {
      ...
      nameRules: [
        ...
        (v) =>
          (v && v.length <= 20) ||
          this.$t("accounts.mdlImportAccount.errorWalletLength"),
      ],
      passRules: [
        (v) =>
          !!v || this.$t("accounts.mdlImportAccount.errorPasswordRequired"),
        (v) =>
          (v && v.length <= 20) ||
          this.$t("accounts.mdlImportAccount.errorPasswordLength"),
      ],
    };
  },
  watch: {
  ...
  methods: {
    ...
    async importWallet() {
      ...
      const foundAddress = this.allWallets.find(
        (element) => element.address === finalAddress[0].address,
      );
      if (foundName) {
